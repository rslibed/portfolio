$(document).ready(function() {
  ...
  $('#contact-form').submit(function() {
    ...
    $(
      '#contact-form .error-message,#contact-form .contact-form-message'
    ).remove();
    ...
    if (hasError) {
      $('#contact-form').append(
        '<p class="contact-form-message">' + errorMessage + '</p>'
      );
      $('#contact-form').removeClass('clicked');
    } else {
      $('#contact-form').append(
        '<p class="contact-form-message"><i class="fa fa-spinner fa-pulse"></i>' +
          sendingMessage +
          '</p>'
      );
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $('#contact-form').append(
            '<p class="contact-form-message">' + okMessage + '</p>'
          );
          $('#contact-form').removeClass('clicked');
        ...
        error: function(data) {
          $('#contact-form .contact-form-message').remove();
          $('#contact-form').append(
            '<p class="contact-form-message">Message can not be sent. Please try again!</p>'
          );
          $('#contact-form').removeClass('clicked');
          $('#contact-form')[0].reset();
          $('#contact-form .form-control').removeClass('input-filled');
